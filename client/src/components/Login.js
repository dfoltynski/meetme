  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        setCookie("user_id", res.data.user_id, { expires: d });

        setUserError(null);
        let imageBase64 = await createImagePreview(res.data.profile_pic.data);
    ...
    <Wrapper>
      <FormContainer style={{ minHeight: "11em" }}>
        ...
        <Form onSubmit={formik.handleSubmit}>
          ...
          />
          {formik.errors.email ? (
            <p className="validation__error">{formik.errors.email}</p>
          ) : null}
          <Input
            name="password"
            type="password"
          ...
          {formik.errors.password ? (
            <p className="validation__error">{formik.errors.password}</p>
          ) : null}
